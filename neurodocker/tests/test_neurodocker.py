...
def test_generate_opts(capsys):
    ...
    out, _ = capsys.readouterr()
    assert ('ENV KEY="VAL" \\' in out and 'KEY="VAL"' in out) or (
        'ENV KEY2="VAL" \\' in out and 'KEY="VAL"'
    )
    ...
    with mock.patch("neurodocker.interfaces.Miniconda._installed", False):
        main(
            args.format(
                "--miniconda create_env=newenv version=4.7.10 "
                "conda_install=python=3.7"
            ).split()
        )
    out, _ = capsys.readouterr()
    assert (
        "https://repo.continuum.io/miniconda/Miniconda3-4.7.10-Linux-x86_64.sh" in out
    )
